...
contract ProfitmaxpresaleV1 is ReentrancyGuard {
    ...
    function updateLevelIncome(
        ...
        for (uint i = user.leafNo; i > 0; i--) {
            ...
            if (user1.rewardPerMinute > 0) {
                ...
                if (timeToMins > 0)
                    users[user.referrer].levelIncomeReceived +=
                        timeToMins *
                        user1.rewardPerMinute;
            }

            users[user.referrer].lastUpdate = block.timestamp;
    ...
    function calculateRewardPerMinute(
        ...
        // console.log("Its Amount To Detect: ", tokens);
        if (tokens == 50000) {
